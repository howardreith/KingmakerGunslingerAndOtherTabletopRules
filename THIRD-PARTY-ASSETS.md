...
The 0.0.63 Windows bundle `kingmakergunslinger.firearms` contains derivatives
of the four approved CC-BY-4.0 models and the five approved SSE CC0 recordings.
...
mono 48 kHz 16-bit PCM. The Martini-Henry/Winchester-identified binary is absent
from staging and from the bundle.

The 0.0.63 Rapid Reload icon is original project artwork generated by the
deterministic editable script `tools/New-RapidReloadIcon.ps1`; it uses only
programmatic geometry and project-selected colors and requires no attribution.
