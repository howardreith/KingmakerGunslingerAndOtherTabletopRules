...
1. Complete semantic-frame/native-donor validation before changing transforms:
   add secondary-axis markers/contracts (`WeaponUp`, `HeadUp`, `BladeNormal`),
...
2. Measure native Light/Heavy Crossbow, Longspear, Scimitar, Bastard Sword, and
   Greatsword donor attachment frames in the real engine. Do not infer donor
   polarity from aggregate renderer size.
3. Qualify and commit/push that diagnostic checkpoint after repository
   validation, all domain tests, clean Release package build, package
   validation, and guarded runtime checks.
4. Continue directly through handgun, long-gun, branched-spear, Eastern held,
...
- Branched-spear physical head is source `+Z`, source head normal is `+Y`, and
  current held/stored X rotations use opposite polarity.
- Eastern physical tip is source `+Z`, cutting edge is `-X`, blade normal is
  `+Y`; identity presentation produces visibly inconsistent in-hand planes.
