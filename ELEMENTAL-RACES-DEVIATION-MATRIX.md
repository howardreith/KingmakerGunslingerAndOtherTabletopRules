...
| Ironsoul Unerring Weapon | Project-owned one-minute-per-level touch weapon effect: +2 on critical-confirmation rolls, plus +1 per four caster levels, to a maximum +7 total confirmation bonus, using a temporary project-owned enchantment on the exact touched held weapon; it adds no weapon damage | PROJECT IMPLEMENTATION; RUNTIME QUALIFICATION PENDING |
| Rimesoul Chill Touch | Project-owned touch delivery with one touch per caster level, native negative-energy damage, Fortitude-negates Strength damage against living targets, and Will-negates fear against undead. Kingmaker's frightened condition is the closest available panicked abstraction. | PROJECT IMPLEMENTATION; FEAR ADAPTATION DOCUMENTED; RUNTIME QUALIFICATION PENDING |
