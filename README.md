...
source-complete branch expects one hash-verified `KMG_Firearms.bnk` generated by
Wwise 2016.2.x. Until that authentic bank and manifest are installed, firearm
mechanics continue normally and the development panel reports audio as
disabled. The retired Unity `AudioSource` fallback is not used.
