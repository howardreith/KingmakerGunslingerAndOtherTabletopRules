...
## Issue 10 - Elven Branched Spear scale and grip

The prior custom prefab was not merely long. Live installed measurement on diagnostic commit `47259f964f072bede2c6c51789b6f73bf9d250cd` proved a complete coordinate-frame mismatch: native `TH_LongspearKnight1` occupied 2.2825 m along local Y, while the custom renderer occupied 2.9235 m along local Z. The semantic Grip/Support/Tip/Butt children were project metadata only and were not consumed by Kingmaker's ordinary Longspear equipment path. Native `WeaponVisualParameters` already supplied `PiercingTwoHanded`, sounds, and attachment behavior without an override.

Published repair `4fb73c18514c05918620238a4d6fa6608ee8cf3d` reauthors every variant to a 2.28 m extent centered on the firing-hand grip. Source remains metric +Z; the Unity builder applies one explicit -90 degree X conversion to the prefab `Visual` so the equipped axis is native +Y. Runtime rejects any different transform, longest axis, or length outside the bounded native contract. The blueprint clone continues to change only `m_WeaponModel`, preserving all mechanics, reach, damage, critical, proficiency, identities, effects, and native presentation fields.

Deterministic generation and package validation cover the editable Blender source, three FBXs, source/runtime icons, Unity 2018.4.10f1 prefab bundle, exact manifest hashes, and provenance. Guarded run `20260820T0733252707402Z-1a9897121438417f95edefbf51d348e5` passed 22/22 with custom/native live longitudinal lengths 2.27855754/2.28250313 m on +Y and exact donor-field equivalence. The save-free view deferred synchronous equipped-instance materialization, so final hand-on-haft and animation appearance remains explicitly human-gated rather than inferred from bounds.

## Issue 11 - Musket and Blunderbuss rig/retexture repair

The prior production bundle imported the preserved licensed FBXs directly and compensated for source units/axes with held scales `4.186` and `20`. It provided semantic muzzle/support/butt children, but no independent validated back-frame contract; the production profile hid long-gun holsters. Fresh human clipping evidence invalidated the earlier minor-clipping acceptance.

Commit `b3b76d74edc27664a0bae538e558b7bd6a15b208` adds a deterministic Blender derivation step while preserving the original CC-BY-4.0 sources. Each derivative is metric, uses +Z barrel orientation, places the firing-hand grip at identity, carries exact Grip/Support/Butt/Muzzle/Back markers, and retains the actual Musket or Blunderbuss geometry with a muted period material tint. The Unity builder produces scale-1 held prefabs plus separately rotated/scaled `MusketBelt` and `BlunderbussBelt` back prefabs. Runtime requires an identity root, `Visual`, `BackMount`, and visible renderer before publishing a custom back model; only then does the project-owned clone clear the inherited crossbow sheath. Crossbow animation, inherited attachment slots, cloned projectile identity, state/audio/reload behavior, and every blueprint identity remain unchanged.

The development calibration surface now exports/applies held visual, muzzle, support, butt, and independent back position/rotation/scale values and refreshes through the native equipment lifecycle. It does not add global race transforms. Deterministic derivative and AssetBundle hashes, source attribution, exact item visual audit, domain tests, clean installed-reference build, strict package validation, and guarded runtime evidence all pass. Run `20260820T0819129064284Z-4b9313e5c2784b099756b97fc139b68e` proved scale-1 held frames, lengths `1.33999968/0.8599998 m`, native support IK, distinct back prefabs, one projectile, custom-back/no-sheath publication, materials, and cleanup. Final clipping, grip, animation, back placement, and texture appearance remain human-gated.

## Issue 12 - Organic project magic-item distribution

The complete scope is 30 stable identities: five rare firearms, eighteen named Eastern weapons, six named Elven Branched Spears, and Cord of Stubborn Resolve. The repair removes all scoped named items from recurring merchant tables and publishes exactly one reference per item to 30 distinct, exact, fixed base-campaign loot targets. Border Sentinel's Issue 7 placement remains unchanged. Existing rare-firearm placements remain unchanged except that stale other project-item clustering at Duelist's Rebuttal's target is removed.
