...
| Durable grapple links and mouth ownership (2026-09-26) | every grabber | `UnitPartSummonGrappleLinks` on the holder | serialized limb slot plus target id; mouth occupancy for held and engulfed victims; reconciliation on every read | the establishing limb survives a save and a reload and the maintain deals its damage, not the first grab limb's; one target per mouth in the attack roll and the planned full attack; the cats' rake is made by the maintain, where the tabletop puts it, because the game's initiator part leaves a holder unable to act | complete; corrected and requalified (correction record) |
