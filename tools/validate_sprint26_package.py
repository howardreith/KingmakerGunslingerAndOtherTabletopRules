#!/usr/bin/env python3
"""Validate the strict standalone 0.0.26 Sprint 26 UMM ZIP."""
...
def main() -> int:
    ...
    with zipfile.ZipFile(args.archive) as archive:
        ...
        if info_json.get("Version") != "0.0.26":
            raise RuntimeError("Info.json does not declare version 0.0.26.")
        ...
        if any(Path(name).name in FORBIDDEN_DLL_NAMES for name in names):
            raise RuntimeError("Archive redistributes a private runtime/compiler/framework reference.")
        if archive.testzip() is not None:
            raise RuntimeError("Archive CRC validation failed.")

    print("Sprint 26 standalone UMM package validation passed.")
