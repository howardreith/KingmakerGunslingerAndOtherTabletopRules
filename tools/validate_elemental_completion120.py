...
def validate(root: Path) -> None:
    ...
    baseline.require_tokens(root / "docs/RELEASE-NOTES-0.0.120.md",
        INFORMATIONAL_VERSION, "owner authorized", "Treacherous Earth remains hidden", "partial Elemental Races completion release")
    ...
    if static.get("elementalCompletion120", {}).get("publicReleaseAuthorized") is not True:
        raise AssertionError("The partial release requires explicit owner authorization.")
    state = static["elementalCompletion120"]
    if state.get("remainingFinalArtifactQualificationWaivedByOwner") is not True or state.get("selectableAlternateTraitCount") != 20 or state.get("treacherousOrdinaryPublication") is not False:
        raise AssertionError("Partial release scope or owner-waived qualification was misrepresented.")
