...
VERSION = "0.0.84"
INFORMATIONAL_VERSION = "0.0.84-urban-barbarian-human-review-repair"
PACKAGE = "KingmakerGunslinger-0.0.84-local-runtime.zip"
...
def validate(root: Path) -> None:
    ...
    required = (
        ...
        "src/KingmakerGunslinger/Blueprints/UrbanBarbarianAllocationIcons.cs",
        "tests/KingmakerGunslinger.DomainTests/UrbanBarbarianInventoryContractTests.cs",
    )
    for relative in required:
        if not (root / relative).is_file():
            raise AssertionError(f"Urban Barbarian repair gate file missing: {relative}")

    project = (root / "src/KingmakerGunslinger/KingmakerGunslinger.csproj") \
        .read_text(encoding="utf-8")
    for include in (
        "RuntimeTesting\\UrbanBarbarianRageInventoryObserver.cs",
        "Blueprints\\UrbanBarbarianAllocationIcons.cs",
        "UrbanBarbarian\\ControlledRageIconPolicy.cs",
    ):
        if include not in project:
            raise AssertionError(f"Urban repair source absent from old-style project: {include}")
