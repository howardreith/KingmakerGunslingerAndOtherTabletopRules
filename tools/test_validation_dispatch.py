...
def main() -> int:
    ...
    sprint44 = source / "tools" / "validate_sprint45.py"

    run(
        [python, "-B", str(dispatcher), "--root", str(source)],
        0,
        "dispatched version 0.0.45 to validate_sprint45.py",
    )
    run([python, "-B", str(sprint44)], 0,
        "Sprint 45 source invariant validation passed")
    ...
    with tempfile.TemporaryDirectory(prefix="KmgValidatorTests-", dir=fixture_parent) as temporary:
        ...
        run(
            ...
            1,
            "Unsupported repository version: '0.0.46'",
        )
