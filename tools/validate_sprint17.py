#!/usr/bin/env python3
"""Portable validation for Sprint 17's executed-evidence and runtime-reference handoff milestone.
...
EXPECTED_INFO = "0.0.17-s17-executed-evidence-handoff"
EXPECTED_TESTS = 373
...
def validate_metadata() -> dict:
    ...
    for token in (
        'AssemblyVersion("0.0.17.0")',
        'AssemblyFileVersion("0.0.17.0")',
        'AssemblyInformationalVersion("0.0.17-s17-executed-evidence-handoff")',
    ):
...
def validate_manifest() -> dict:
    ...
    assert len(active) == 8
    assert not any(entry.get("milestone") == "Sprint 17" and entry.get("status") == "active" for entry in entries)
    return {
        "passed": True,
        "entryCount": 12,
        "activeCount": 8,
        "reservedCount": 4,
        "newSprint17BlueprintIds": 0,
...
def validate_evidence_and_fixture_source() -> dict:
    ...
    for token in (
        ...
        'Note = "Trusted Sprint 17 runtime preflight: "',
        "Before = null",
        "After = null",
...
def validate_runtime_contract_script() -> dict:
    ...
    return {"passed": True, "sprint": 17, "identityContractRequired": True}


def validate_executed_evidence_and_handoff() -> dict:
    evidence = json.loads(read("evidence/executed-domain-tests/executed-test-evidence.json"))
    assert evidence["targetFrameworkReferenceSurface"] == ".NET Framework 4.7"
    assert evidence["languageVersion"] == "7.3"
    assert evidence["warningsAsErrors"] is True
    assert evidence["compileExitCode"] == 0
    assert evidence["declaredTests"] == EXPECTED_TESTS
    assert evidence["runs"] == 3
    assert evidence["failures"] == 0
    assert evidence["repeatedOutputIdentical"] is True
    for key in ("testExecutableSha256", "testOutputSha256", "sourceSetSha256", "compilerSha256"):
        assert re.fullmatch(r"[0-9a-f]{64}", evidence[key]), key
    output_paths = [ROOT / f"evidence/executed-domain-tests/run{number}.stdout.txt" for number in range(1, 4)]
    outputs = [path.read_text(encoding="utf-8") for path in output_paths]
    assert len(set(outputs)) == 1
    assert outputs[0].splitlines()[-1] == f"Completed {EXPECTED_TESTS} tests; failures=0."
    assert sha256(output_paths[0]) == evidence["testOutputSha256"]

    test_project = ROOT / "tests/KingmakerGunslinger.DomainTests/KingmakerGunslinger.DomainTests.csproj"
    source_digest = hashlib.sha256()
    for include in sorted(compile_items(test_project)):
        source_path = (test_project.parent / include.replace("\\", "/")).resolve()
        source_digest.update(str(source_path.relative_to(ROOT)).encode("utf-8"))
        source_digest.update(b"\0")
        source_digest.update(source_path.read_bytes())
        source_digest.update(b"\0")
    assert source_digest.hexdigest() == evidence["sourceSetSha256"]
    for number in range(1, 4):
        assert read(f"evidence/executed-domain-tests/run{number}.stderr.txt") == ""

    identity = read("src/KingmakerGunslinger/Firearms/FirearmItemId.cs")
    assert "!ReferenceEquals(other, null)" in identity
    assert "ReferenceEquals(other, null)" in identity
    assert "!ReferenceEquals(left, null)" in identity
    armor = read("src/KingmakerGunslinger/Rules/FirearmArmorClassService.cs")
    for token in (
        ...
        assert token in armor, token
    tests = read("tests/KingmakerGunslinger.DomainTests/Program.cs")
    for token in (
        ...
        "no existing firearm-state entry",
    ):
        assert token in tests, token

    exporter = read("scripts/export-private-build-references.ps1")
    for token in (
    ...
    builder = read("tools/build_mod_from_private_references.py")
    for token in (
        "compile-candidate-not-runtime-qualified",
        '"readyForKingmakerSmokeTest": True',
        '"persistenceGateDecision": "NoGoIncomplete"',
        "KingmakerGunslinger-private-build-references",
    ):
        assert token in builder, token
    runner = read("tools/run_exact_net47_domain_tests.py")
    for token in (
        '"/langversion:7.3"',
        '"/warnaserror+"',
        '"/deterministic+"',
        "Repeated test output was not byte-identical",
    ):
        assert token in runner, token
    return {
        "passed": True,
        "compiledTests": EXPECTED_TESTS,
        "executedRuns": 3,
        "failures": 0,
        "privateReferenceFiles": 10,
    }


def validate_documents() -> dict:
    required = {
        "SPRINT-17-REPORT.md": ("executed evidence", "373/373", "NoGoIncomplete"),
        "README.md": (EXPECTED_INFO, "373", "not UMM-installable"),
        "TESTING.md": ("NOT READY FOR KINGMAKER", "Completed 373 tests", "export-private-build-references.ps1"),
        "KNOWN-ISSUES.md": ("No UMM-installable package", "Persistence remains incomplete"),
        "docs/EXECUTED-TEST-EVIDENCE.md": ("373", "Recursive firearm-item equality", "Invalid attack distance failed open"),
        "docs/PRIVATE-RUNTIME-REFERENCE-BUNDLE.md": ("ten managed assemblies", "must not be published"),
        ...
        "docs/decisions/ADR-0024-executed-evidence-and-private-reference-handoff.md": ("Accepted for Sprint 17", "373"),
        "planning/SPRINT-18-ENTRY-CRITERIA.md": ("Branch A", "Branch B", "Branch C"),
        "docs/history/SPRINT-16-REPORT.md": ("Sprint 16", "371"),
        "docs/history/SPRINT-17-ENTRY-CRITERIA.md": ("Branch A", "Branch B", "Branch C"),
...
def main() -> int:
    ...
    checks["executedEvidenceAndHandoff"] = validate_executed_evidence_and_handoff()
    ...
    print(
        f"Sprint 17 portable validation passed: {EXPECTED_TESTS} declared tests, "
...
if __name__ == "__main__":
    try:
        ...
        print(f"Sprint 17 validation failed: {exc}", file=sys.stderr)
        raise
