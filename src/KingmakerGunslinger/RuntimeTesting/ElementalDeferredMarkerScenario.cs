            ...
            try {
                ...
                for (int sex = 0; sex < 2; sex++) {
                    ...
                    foreach (var trait in race.AlternateTraits.Traits()) {
                        ...
                        bool inertOldEarth = authored && trait.Definition.Id == ElementalAlternateTraitId.TreacherousEarth;
                        fact(trait.Marker, inertOldEarth ? 1 : active); fact(trait.Provider, active);
                        ...
                        if (inertOldEarth) {
                            var marker = owner.Progression.Features.Enumerable.Single(value => ReferenceEquals(value.Blueprint, trait.Marker));
                            var logic = marker.SelectComponents<ElementalAlternateTraitMarkerController>().Single();
                            var revision = typeof(ElementalAlternateTraitMarkerController).GetField("_completionRevision",
                                BindingFlags.Instance | BindingFlags.NonPublic);
                            exact &= revision != null && (int)revision.GetValue(logic) == 0 && !logic.IsEffective;
                        }
                    }
