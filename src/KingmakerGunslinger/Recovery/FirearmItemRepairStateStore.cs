...
{
    /// <summary>
    /// Exact runtime-item adapter over the accepted item-owned state-token service for
    /// ordinary Broken-to-Normal repair.
    /// </summary>
    internal sealed class FirearmItemRepairStateStore : IFirearmRepairStateStore
    {
        private readonly FirearmItemStateService _service;
        private readonly object _item;

        internal FirearmItemRepairStateStore(
            ...
            FirearmState replacement)
        {
            ...
            if (current != expectedCurrent)
            {
                throw new InvalidOperationException(
                    "The exact firearm changed before ordinary repair state replacement.");
            }
            ...
            {
                throw new InvalidOperationException(
                    "The exact firearm did not retain the requested ordinary repair state.");
            }
        }
    }
}
