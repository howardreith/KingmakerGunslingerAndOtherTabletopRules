...
{
    /// <summary>
    /// Correlates one ordinary-repair transaction with exact repository snapshots that
    /// prove same-item identity and exactly one successful state revision.
    /// </summary>
    internal sealed class FirearmRepairRuntimeResult
    {
        internal FirearmRepairRuntimeResult(
            ...
            FirearmItemStateSnapshot afterFirearm)
        {
                    ...
                    AfterFirearm.Repository.RuntimeReferenceHash)
            {
                throw new ArgumentException(
                    "An ordinary repair runtime result must preserve exact repository and runtime-reference identity.");
            }
            ...
            {
                throw new ArgumentException(
                    "Ordinary repair transaction states must match the exact item repository snapshots.");
            }
            ...
            {
                throw new ArgumentException(
                    "A successful ordinary repair must advance the exact item revision once; a rejection must not advance it.");
            }
        }

        internal FirearmRepairResult Transaction { get; private set; }
