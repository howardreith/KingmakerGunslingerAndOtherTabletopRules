...
{
    internal sealed class TargetingArmsBlueprintSet
    {
        internal TargetingArmsBlueprintSet(BlueprintFeature feature,
    ...
    internal static class TargetingArmsBlueprints
    {
        internal const string FeatureSymbol = "KMG.Deeds.TargetingArmsFeature";
        internal const string AbilitySymbol = "KMG.Deeds.TargetingArmsAbility";

        internal static TargetingArmsBlueprintSet Register(BlueprintRegistry registry)
        {
            ...
            return new TargetingArmsBlueprintSet(feature, ability);
        }

        private static BlueprintAbility CreateAbility()
        {
            var result = ScriptableObject.CreateInstance<BlueprintAbility>();
            result.name = "KMG_TargetingArms_Ability";
            BlueprintUnitFactAccess.Resolve().Configure(result,
                LocalizationService.Create("KMG.TargetingArms.Ability.Name",
                    "Targeting — Arms"),
                LocalizationService.Create("KMG.TargetingArms.Ability.Description",
                    "Spend 1 grit as a full-round action to make one firearm attack that deals no damage. On a hit, an eligible target cannot use its main-hand item for one round."), null);
            ...
            var logic = ScriptableObject.CreateInstance<TargetingArmsAbilityLogic>();
            logic.name = "$KMG_TargetingArms_Delivery";
        ...
        private static BlueprintFeature CreateFeature(BlueprintAbility ability)
        {
            ...
            BlueprintUnitFactAccess.Resolve().Configure(result,
                LocalizationService.Create("KMG.TargetingArms.Feature.Name",
                    "Targeting — Arms"),
                LocalizationService.Create("KMG.TargetingArms.Feature.Description",
                    "Make a no-damage firearm attack that briefly disables the target's main-hand item."), null);
            return result;
        }
            ...
            BlueprintAbility ability)
        {
            ...
            if (!ability.IsFullRoundAction || ability.Range != AbilityRange.Weapon ||
                ...
                ability.ComponentsArray.OfType<TargetingArmsAbilityLogic>().Count() != 1)
                throw new InvalidOperationException(
                    "Targeting Arms blueprint contract is incomplete.");
        }
    }
}
