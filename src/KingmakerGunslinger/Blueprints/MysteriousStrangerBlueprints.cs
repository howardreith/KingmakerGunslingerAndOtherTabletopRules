    ...
    internal static class MysteriousStrangerBlueprints
    {
            ...
            BlueprintFeatureSelection gunTraining, BlueprintFeature bleedingWound)
        {
            ...
            BlueprintBuff focusedBuff=registry.Register<BlueprintBuff>("KMG.Archetypes.FocusedAimBuff",
                ()=>CreateBuff("FocusedAim", new FocusedAimDamage()));
            ...
            BlueprintBuff fortuneBuff=registry.Register<BlueprintBuff>("KMG.Archetypes.StrangersFortuneArmed",
                ()=>CreateBuff("StrangersFortune", null));
            ...
            BlueprintBuff clippingBuff=registry.Register<BlueprintBuff>("KMG.Archetypes.ClippingShotArmed",
                ()=>CreateBuff("ClippingShot", new ClippingShotAttackHandler{Grit=baseGrit.Resource}));
        ...
        private static BlueprintBuff CreateBuff(string name,BlueprintComponent component){var b=ScriptableObject.CreateInstance<BlueprintBuff>();b.name="KMG_"+name+"_Buff";b.Stacking=StackingType.Replace;b.IsClassFeature=true;b.ComponentsArray=component==null?Array.Empty<BlueprintComponent>():new[]{component};return b;}
        private static BlueprintAbility CreateArmAbility(string name,BlueprintBuff marker,BlueprintAbilityResource resource,int cost,bool spend){var a=ScriptableObject.CreateInstance<BlueprintAbility>();a.name="KMG_"+name.Replace(" ","").Replace("'","")+"_Ability";a.Type=AbilityType.Extraordinary;a.Range=AbilityRange.Personal;a.CanTargetSelf=true;a.ActionType=name=="Focused Aim"?UnitCommand.CommandType.Swift:UnitCommand.CommandType.Free;a.Animation=UnitAnimationActionCastSpell.CastAnimationStyle.Self;a.ResourceAssetIds=Array.Empty<string>();var logic=ScriptableObject.CreateInstance<ArmMysteriousStrangerDeed>();logic.Marker=marker;logic.Resource=resource;logic.Cost=cost;logic.SpendOnActivation=spend;a.ComponentsArray=new BlueprintComponent[]{logic};BlueprintUnitFactAccess.Resolve().Configure(a,LocalizationService.Create("KMG."+name+".Ability.Name",name),LocalizationService.Create("KMG."+name+".Ability.Description","Activate "+name+"."),null);return a;}
