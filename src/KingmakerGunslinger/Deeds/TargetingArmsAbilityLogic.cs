...
{
    [Serializable]
    public sealed class TargetingArmsAbilityLogic : AbilityCustomLogic,
        IAbilityAvailabilityProvider
    {
        public bool IsAvailableFor(AbilityData ability)
        {
            ...
            return TargetingArmsRuntime.Evaluate(ability.Caster, true,
            ...
            AbilityExecutionContext context, TargetWrapper target)
        {
            if (target == null || target.Unit == null)
                throw new InvalidOperationException(
                    "Targeting Arms requires a unit target.");
            TargetingArmsRuntime.Execute(context, target.Unit);
