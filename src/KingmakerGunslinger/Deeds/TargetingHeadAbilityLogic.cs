...
{
    [Serializable]
    public sealed class TargetingHeadAbilityLogic : AbilityCustomLogic,
        IAbilityAvailabilityProvider
    {
        public BlueprintBuff ConfusionBuff;
        public bool IsAvailableFor(AbilityData ability)
        {
            if (ability == null || ability.Caster == null) return false;
            Actions.ExactEquippedFirearmContext ignored; string reason;
            return TargetingHeadRuntime.Evaluate(ability.Caster, true,
            ...
            AbilityExecutionContext context, TargetWrapper target)
        {
            if (target == null || target.Unit == null)
                throw new InvalidOperationException("Targeting Head requires a unit target.");
            TargetingHeadRuntime.Execute(context, target.Unit, ConfusionBuff);
