...
namespace KingmakerGunslinger.Spells.Teleportation
{
    ...
    internal enum TeleportCastSourceFacts
    {
        ...
        RequiredScroll = ActiveParty | LivingAvailableCaster | ScrollStock | ExactSpell | RealResource
    }
    ...
    internal static class TeleportCastAvailabilityPolicy
    {
        internal static bool Usable(TeleportCastSourceSnapshot source)
        {
            if (source == null || string.IsNullOrWhiteSpace(source.CasterId) || source.CasterId.Contains("/") ||
                ...
                source.SpellLevel > 9 || !Enum.IsDefined(typeof(TeleportSpellKind), source.Spell)) return false;
            if (source.Kind == TeleportCastSourceKind.Scroll)
            {
                // Inventory-backed sources need no spellbook, slot or preparation;
                // only the shared scroll stock and a legitimate reader.
                return source.Has(TeleportCastSourceFacts.RequiredScroll) &&
                    (source.Facts & (TeleportCastSourceFacts.OwnedSpellbook | TeleportCastSourceFacts.BookUsable |
                        TeleportCastSourceFacts.Known | TeleportCastSourceFacts.PreparedUse |
                        TeleportCastSourceFacts.Metamagic | TeleportCastSourceFacts.Synthetic)) == 0;
            }
            if (!source.Has(TeleportCastSourceFacts.Required) ||
                (source.Facts & (TeleportCastSourceFacts.Item | TeleportCastSourceFacts.Metamagic |
                    TeleportCastSourceFacts.Synthetic | TeleportCastSourceFacts.ScrollStock)) != 0) return false;
