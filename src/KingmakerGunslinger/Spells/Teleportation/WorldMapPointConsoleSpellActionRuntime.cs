    ...
    internal sealed class TeleportConsoleDestinationRows : MonoBehaviour
    {
        ...
        private void Choose(Row row)
        {
            try
            {
                ...
                if (fresh == null) { _ready = false; TeleportContextConfirmationPresenter.Unavailable(row.Action, context); WorldMapPointConsoleSpellActionRuntime.Clear(_panel); return; }
        ...
        private void Resize()
        {
            ...
            float content = _rows.Count * _rowHeight + _separators.Count * TeleportationUiDivider.SeparatorHeight;
