...
The append-only contract contains 214 stable identifiers: 213 active and one reserved. Prior wrapper and persisted marker identities remain hidden for compatibility; the latest identities append scoped firearm-proficiency facts, visible archetype proficiency grants, Exotic Weapon Proficiency (Firearms), and rankable Pistol/Musket Training facts without changing any established GUID.
