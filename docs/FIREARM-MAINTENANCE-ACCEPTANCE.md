...
| --- | --- | --- | --- | --- |
| F01 | Broken gun, legitimate repair capability, outside combat, reusable kit: full-round repair succeeds on the same exact item; rounds/ammunition/enchantments/origin survive; kit/gold/grit/crafting allowance unchanged. | PARTIAL 2026-09-12: Broken+kit+non-combat policy domain-tested; transaction same-item/rounds/kit-untouched covered by Sprint29 suite + field-repair tests. | NOT RUN | |
| F02 | Normal gun, missing kit, only a retired consumable kit, missing capability, incapable character, or ambiguous/no equipped target: rejected without state/resource mutation. Multiple reusable kits still satisfy at-least-one. | PARTIAL 2026-09-12: Normal/missing-kit rejection + Wrecked rest-only domain-tested (field-repair.normal-and-missing-kit-rejected, wrecked-rest-only; Sprint30 generic cases). | NOT RUN | |
| F03 | Active combat in RTWP and turn-based, including a party encounter where the caster is not personally engaged: availability and execution reject. | PARTIAL 2026-09-12: party-level combat authority wired (Player.IsInCombat; field-reparty-combat-authority); both combat modes need native runs. | NOT RUN | |
| F04 | Eligible start, then combat begins / kit lost / capability lost / caster unable to act / command cancelled / original weapon or context changes before delivery: original and replacement guns remain unrepaired. | PARTIAL 2026-09-12: command-start binding wired (UnitUseAbility.OnStart/OnEnded + delivery ReferenceEquals; field-repair.command-start-binding); interruption-before-delivery needs native runs. | NOT RUN | |
| F05 | Wrecked gun via ordinary field repair, direct ability delivery, and legacy Overhaul fact/slot: rejected in and out of combat with full-rest explanation. | PARTIAL 2026-09-12: Wrecked rejected by policy AND transaction with rest-only reason (field-repair.wrecked-rest-only, transaction-rejects-wrecked); legacy alias delegates to the same logic; native runs pending. | NOT RUN | |
| F06 | Ordinary success and injected state-write/verification failure: exact-item transaction/rollback correct; no inventory/resource side effects; no false success notification. | NOT RUN (Sprint29 rollback/fault-injection suite still covers the transaction layer; re-verified in the 1611-run) | NOT RUN | |
