...
The blueprint ledger contains 31 stable IDs: 30 active and one reserved. Firearm Proficiency grants Reload, Overhaul, and Repair. The Gunslinger level-one progression grants a persistent per-unit grit resource whose maximum is its one-point base floor plus the exact Wisdom amount above that floor; a hidden per-unit marker makes post-level initialization one-time across later multiclass reapplication. The standalone package continues to contain exactly one project-owned binary and no private reference assembly.
