...
The blueprint ledger contains 153 stable IDs: 152 active and one reserved, for one-hundred-fifty-two active blueprints. Playtest 0.0.61 adds four hidden command-cost variants beneath the stable Reload Firearm parent plus grouped firearm Weapon Focus and Rapid Reload selections with five choices each. Earlier stable identities remain append-only. The standalone package continues to contain exactly one project-owned binary and no private reference assembly.
