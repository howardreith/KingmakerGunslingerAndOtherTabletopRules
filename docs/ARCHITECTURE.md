...
The blueprint ledger contains 141 stable IDs: 140 active and one reserved, for one-hundred-forty active blueprints. Playtest 0.0.61 adds four hidden command-cost variants beneath the stable Reload Firearm parent. Earlier stable identities remain append-only. The standalone package continues to contain exactly one project-owned binary and no private reference assembly.
