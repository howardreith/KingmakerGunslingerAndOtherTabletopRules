...
The blueprint ledger contains 159 stable IDs: 158 active and one reserved, for one-hundred-fifty-eight active blueprints. Playtest 0.0.62 adds one native Weapon Focus wrapper while preserving all 0.0.61 reload variants, firearm feat choices, and presentation-only deed-tier summaries. Earlier stable identities remain append-only. The standalone package continues to contain exactly one project-owned binary and no private reference assembly.
