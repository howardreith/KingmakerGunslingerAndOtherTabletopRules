...
Sprint 59 extends the ledger to 126 stable IDs: 125 active and one reserved.
...
The blueprint ledger contains 126 stable IDs: 125 active and one reserved, for one-hundred-twenty-five active blueprints. Sprint 59 adds True Grit's repeated level-20 selection and eighteen stable unit-owned deed choices. Sprint 58 adds Stunning Shot's feature, arming ability, unit-owned marker, and exact native Stunned clone. Sprint 56 adds Cheat Death's level-19 feature and completed-damage handler. Sprint 55 adds Slinger's Luck's feature, two fixed-cost arming abilities, and two unit-owned reroll markers. Sprint 54 adds Menacing Shot's feature and exact native-Fear-derived ability. Sprint 53 adds Evasive's controller and three exact native-mechanics clones. Sprint 52 adds Lightning Reload's feature, swift-action ability, and unit-local round-use marker. Sprint 51 adds Expert Loading's feature, pre-shot ability, and armed marker. Sprint 50 adds Bleeding Wound's feature, four pre-shot choices, four armed markers, and four persistent bleed facts. Sprint 44 adds Startling Shot's level-seven feature, standard-action ability, and one-round native flat-footed buff. Sprint 43 adds Dead Shot's level-seven feature and full-round ability. Its short-lived weak contexts distinguish BAB-derived native probe rolls from the one item-owned discharge, aggregate hits, threats, and all-roll misfire, override only the primary weapon's base dice, and retain one native critical confirmation. Sprint 42 adds one cumulative Gun Training selection and five exact firearm-kind facts; their native rule handlers add Dexterity damage only for a matching immutable firearm marker, while the misfire adapter distinguishes the ordinary Broken +4 threshold from trained +2. Firearm Proficiency grants Reload, Overhaul, and Repair. The Gunslinger level-one progression grants a persistent per-unit grit resource whose maximum is its one-point base floor plus the exact Wisdom amount above that floor; a hidden per-unit marker makes post-level initialization one-time across later multiclass reapplication. Sprint 36 adds Deadeye, the adapted drop-prone branch of Gunslinger's Dodge, and Quick Clear's exact standard/move action pair. Sprint 37 adds five cumulative native Dodge-descriptor Nimble facts at levels 2/6/10/14/18, conditionally applied in light or no armor. Sprint 38 adds a level-three unit-owned initiative handler that changes only the native initiative modifier and only while current grit is positive. Sprint 39 adds Pistol-Whip's feature/ability plus four hidden transient melee-surrogate identities; the surrogates are never equipped or persisted. Sprint 40 adds Utility Shot's feature and Stop Bleeding ability, using a no-roll one-chamber discharge and native Bleed fact removal. The standalone package continues to contain exactly one project-owned binary and no private reference assembly.
