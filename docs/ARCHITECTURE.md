...
The blueprint ledger contains 184 stable IDs: 183 active and one reserved, for one-hundred-eighty-three active blueprints. Playtest 0.0.63 retains the 0.0.61/0.0.62 wrapper identities as hidden compatibility records, publishes firearm parameters inside the five native weapon-feat menus, exposes one dynamic Reload Firearm ability, and adds the timed Gunslinger's Dodge AC buff. Earlier stable identities remain append-only. The standalone package continues to contain exactly one project-owned binary and no private reference assembly.
