...
The blueprint ledger contains 183 stable IDs: 182 active and one reserved, for one-hundred-eighty-two active blueprints. Playtest 0.0.62 adds native-style firearm wrappers for Weapon Focus and its four dependent feat families while preserving all 0.0.61 reload variants, firearm feat choices, and presentation-only deed-tier summaries. Earlier stable identities remain append-only. The standalone package continues to contain exactly one project-owned binary and no private reference assembly.
