...
Kingmaker Gunslinger `0.0.100` targets:
...
```powershell
gh release download v0.0.100 `
  --repo howardreith/KingmakerGunslingerAndOtherTabletopRules `
  --pattern 'KingmakerGunslinger-0.0.100-craft-magic-items-post-human-refinement.zip' `
  --dir "$env:USERPROFILE\Downloads\KingmakerGunslinger-0.0.100"
```
