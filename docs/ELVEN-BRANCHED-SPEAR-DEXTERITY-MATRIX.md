...
The final isolated Call of the Wild combat qualification is guarded Steam run
`20260814T0332236294874Z-disposable-elven-branched-spear-combat` from commit
`f8dbcb98485c385b7365e206ec150bb2ded9aa0a`. It used three request-local live
units, equipped item entities, native and optional facts, and actual attack/stat
rule events. The three final working-save phases are listed below; registration
evidence is not substituted for behavior tests.
...
| Rogue Finesse Training `b78d146cea711a84598f0acef69462ea` | Native rogue class feature | Static selection children use `WeaponTypeDamageStatReplacement`; native Elven Curve Blade child has `OnlyOneHanded=false`, `TwoHandedBonus=true` | Qualifies through the ordinary spear child; every item shares the category | Idempotently publish `KMG.ElvenBranchedSpear.FinesseTraining` | Exactly one option; DEX damage on all variants; remove/reselect and save/load; native two-handed multiplier comparison | Runtime PASS: all 12 variants, remove/reselect, module-OFF load, and final cleanup |
| A class/archetype reusing native Finesse Training | Native or optional content | Same native selection and exact category replacement child | Qualifies if it grants/selects the native parent without adding another restriction | No extra child or bespoke feature | The shared selected child is added once and exercised on a live unit | PASS through the common native child; no distinct compatible owner implementation was found |
...
| Multiple category replacement facts | Native `WeaponTypeDamageStatReplacement` | Exact `WeaponCategory`, `OnlyOneHanded`, and native replacement arbitration | Qualifies only when the concrete fact names the spear category and allows two-handed use | Publish only the ordinary Finesse Training child; the second fact is request-local test data only | Two equivalent facts do not double; switching to a native Longspear clears replacement | Runtime PASS: one DEX x1.5 source; native Longspear restored STR x1.5 |
| Fighter's Finesse `c790786d2e2349ff9f6f20731a7c425a` | Call of the Wild advanced weapon training | `AttackStatReplacementIfWeaponTraining(Dexterity)`; requires Weapon Finesse and a qualifying weapon-training group | Qualifies through the native Spears fighter group | No hard dependency or selector copy; shared type has `WeaponFighterGroup.Spears` | DEX attack with Spears training; damage remains governed by independent sources | Isolated runtime PASS: DEX attack, STR x1.5 damage |
| Trained Grace `3bf81c936aac4e039eaa2ec032a34584` | Call of the Wild advanced weapon training | Adds trained-grace weapon-training value; it is not a generic DEX damage-stat replacement | Applies only under its own weapon-training and finesse rules | None | Preserve native auxiliary bonus without reclassifying it as DEX damage | Isolated runtime PASS: STR x1.5 remained; native auxiliary bonus increased by 1 |
...
| Fencing Grace `47b352ea0f73c354aba777945760b441` | Native feat | `WeaponSubCategory.OneHandedPiercing`, free-hand/Grace semantics | Two-handed spear is ineligible | Do not add spear or weaken category | Not offered; cannot apply | Runtime PASS: zero spear options |
| Slashing Grace `697d64669eb2c0543abb9c9b07998a38` | Native feat | `WeaponSubCategory.OneHandedSlashing`, Grace semantics | Piercing two-handed spear is ineligible | Do not add spear or weaken category | Not offered; cannot apply | Runtime PASS: zero spear options |
| Dervish Dance `a18c439e0cca4232a067d13e6401d925` | Call of the Wild feat | Scimitar proficiency plus `DamageGraceForWeapon(Scimitar)` | Wrong named category and handedness contract | None | Spear absent and rejected | Isolated runtime PASS: spear damage remained STR x1.5 |
...
Final isolated run
`20260814T0332236294874Z-disposable-elven-branched-spear-combat` passed all 17
assertions with zero warnings. With Strength 10, Dexterity 20, and BAB 12, the
live observations were:
...
- selection removal: Strength x1.5, bonus 0; exact reselection: Dexterity x1.5,
  bonus 7;
- two equivalent category facts: one Dexterity x1.5 source; switching to a
  native Longspear: Strength x1.5;
- isolated Call of the Wild: Fighter's Finesse Dexterity attack/Strength x1.5
  damage, Trained Grace +1 native auxiliary damage, Dervish Dance no effect;
- Fencing Grace and Slashing Grace: zero spear options.
...
The clean-commit working-save sequence
...
proved that all 12 items and the exact ordinary Finesse Training child survive a
fresh load with the module disabled. Cleanup removed both, wrote the disposable
working save once, and a final fresh load observed zero items, zero selected
facts, and zero save writes. The combat fixture separately proves native
respec-rebuild semantics at the exact feature remove/reselect boundary; it does
not automate visual level-up UI navigation.
