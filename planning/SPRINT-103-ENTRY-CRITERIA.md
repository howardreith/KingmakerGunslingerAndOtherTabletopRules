...
does not retain the player-sold item for repurchase. Stage the ordinary
production pistol from the request-local generated catalog, commit its player
purchase, and prove it does not inherit the sold battered origin. Temporary
request-local funding is allowed only when required and
must be reversed through the native money API. The final invariant requires the
player inventory, money, class,
