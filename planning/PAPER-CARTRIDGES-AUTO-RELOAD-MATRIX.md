...
| `BlueprintBootstrap` / manifests / project files | Baseline 242 registrations and 243 stable ledger rows | Item, two tokens, activatable, and marker appended so far: 247 active / 248 total including one reserved; paper crafting identity remains Phase 5 |
| Manual reload integration | Loose-only availability/action/delivery used separate calculations | COMPLETE: one exact plan drives availability, `AbilityData`, `UnitUseAbility`, delivery, generic atomic source consumption, native per-unit mode, and no-fallback rejection; guarded run `20260808T2233133870047Z-7cc71df785694c1bbdb6d3a747cb87d7` PASS |
