...
| ACQUISITION | 30 targets remain clustered/questionable | implemented; source-qualified; runtime-qualified; human-gated | 25/25 graph run proved reference shape, not organic pacing or normal lootability | revised fixed map reduces maximum normalized named-area density from six to two and preserves one exact target per unique | human-check ordinary accessibility, theme, and pacing through the read-only locator; do not claim graph membership proves player experience |
...
| ICONS | Firearm monograms and Rapid Reload remain aesthetically wrong | implemented; source-qualified; runtime-qualified; human-gated | semantic identity/contact sheets did not match accepted Nodachi/native feat grammar | stable choice blueprints now use deterministic pale Nodachi-like calligraphic P/M/B/Ri/Rv art; Rapid Reload uses a separate native-feat-style pale/oxblood glyph | compare contact sheets and actual 32/64 UI beside Nodachi/native feats |
| LONG-GUN-YAW | Musket and Blunderbuss point too far left | implemented; source-qualified; runtime-qualified; human-gated | structural 63/63 rig run did not measure apparent yaw on dolls | smallest held-only corrections are Musket `+3` and Blunderbuss `+4` local Y; all accepted rig/back/mechanics fields remain unchanged | human-check world/inventory dolls, bodies, direct/Scatter, attack/reload/switch/back states |
