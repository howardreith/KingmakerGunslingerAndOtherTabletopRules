...
| ACADEMAE | Toggle-on prepared Summon Monster I still uses full-round; no save/fatigue | reported; prior completion rejected | synthetic 14/14 run did not exercise the ordinary player command path | eligibility or action override rejects real AbilityData/slot/command state before completion correlation | instrument exact real prepared command fields and rejection reason |
...
| BTSL | Permanent equipment is on Xelliren instead of Honest Guy | reported | prior publication tests did not assert owner responsibility or natural ordering | project rows target both exact dragon tables without category partition | audit current four exact tables and sorting contract, then migrate owned rows transactionally |
