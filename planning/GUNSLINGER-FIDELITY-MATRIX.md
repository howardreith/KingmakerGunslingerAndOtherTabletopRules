...
| 1 | Grit: Wisdom-based pool, daily reset, critical/killing-blow recovery | EXACT | Runtime-qualified stable per-unit resource initializes once after exact level reapply, preserves spent value on later multiclass reapplies, spends atomically, refills on native rest, round-trips exact blueprint/current through native persistence JSON, and follows the native respec-list transfer contract | Implement firearm critical/killing-blow recovery with event dedupe |
...
| 1 | Quick Clear | ADAPTED | Not implemented | Exact equipped misfire-broken firearm; standard no-cost or move action for 1 grit |
| 2/6/10/14/18 | Nimble +1..+5 | EXACT | Not implemented | Light/no-armor dodge AC progression |
| 3 | Gunslinger Initiative | ADAPTED | Not implemented | +2 initiative while grit-positive; native auto-draw semantics require contract review |
