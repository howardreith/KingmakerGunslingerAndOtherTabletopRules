...
| 1 | Grit: Wisdom-based pool, daily reset, critical/killing-blow recovery | EXACT | Runtime-qualified stable per-unit resource initializes after exact level reapply to the Wisdom-floor maximum, spends atomically, does not refill later levels, refills on native rest, and round-trips exact blueprint/current through native persistence JSON | Qualify explicit multiclass/respec value behavior, then firearm event dedupe |
