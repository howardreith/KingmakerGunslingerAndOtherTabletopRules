...
| 1 | Grit: Wisdom-based pool, daily reset, critical/killing-blow recovery | EXACT | Source-qualified bounded pool and stable per-unit Kingmaker resource implement Wisdom modifier minimum 1, initial restore, no level-up refill, atomic spend/restore, clamping, and per-unit operation dedupe; runtime, rest, persistence, and firearm recovery evidence remain pending | Run detached resource acceptance, then qualify rest/save-load and firearm event dedupe |
