...
| 1 | Grit: Wisdom-based pool, daily reset, critical/killing-blow recovery | EXACT | Runtime-qualified stable per-unit resource initializes to the Wisdom-floor maximum, spends atomically, does not refill on level-up, and restores capped; bounded daily reset and dedupe are source-qualified | Qualify rest/save-load, multiclass/respec persistence, then firearm event dedupe |
