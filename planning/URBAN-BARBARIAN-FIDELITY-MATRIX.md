...
| Crowd Control AC | +1 dodge adjacent to 2+ enemies | +1 dodge AC through the defender/initiator-scoped `RuleCalculateAC` event; installed constructor metadata identifies its first `initiator` argument as the defender | Focused repair pending rerun |
