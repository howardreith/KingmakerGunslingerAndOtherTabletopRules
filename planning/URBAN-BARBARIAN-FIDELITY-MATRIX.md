...
| Crowd Control AC | +1 dodge adjacent to 2+ enemies | +1 dodge AC through the defender/target-scoped `RuleCalculateAC` event. Exact `OnTrigger` IL reads `RulebookTargetEvent.Target.Stats.AC`; the synchronous temporary modifier refreshes native AC before rule resolution. | Focused fixture repair pending rerun |
| Crowd adjacency | Adjacent hostile crowd | Live 0.0.83 observation proved `UnitEntityData.DistanceTo` reports center-to-center distance for the request-local units. The implementation converts it to native edge distance with `max(0, DistanceTo - owner.Corpulence - candidate.Corpulence)`, then applies 1.524 m plus float tolerance; weapon reach is never read. | Center/edge distinction observed at runtime; corrected candidate pending rerun |
