...
Status: **0.0.83 HUMAN-REVIEW REJECTED; 0.0.84 RUNTIME FAILED; 0.0.85 EXACT AC
SOURCE-ATTRIBUTION REPAIR IN PROGRESS**.
...
| Crowd Control AC | +1 dodge adjacent to 2+ enemies | +1 dodge AC through the defender/target-scoped `RuleCalculateAC` event. Exact `OnTrigger` IL reads `RulebookTargetEvent.Target.Stats.AC`; the synchronous temporary modifier refreshes native AC before rule resolution. The repaired rule also records the exact Crowd Control fact once in `BonusSources` for combat-log attribution without applying a second bonus. | 0.0.84 proved exact +1 final AC but failed source attribution; 0.0.85 rerun pending |
