...
OBSERVER FAILED; 0.0.86 BOUNDARY OBSERVER FAILED; 0.0.87 AUTOMATED
...
| Crowd Control AC | +1 dodge adjacent to 2+ enemies | +1 dodge AC through the defender/target-scoped `RuleCalculateAC` event. Exact `OnTrigger` IL reads `RulebookTargetEvent.Target.Stats.AC`; the synchronous temporary modifier refreshes native AC before rule resolution. The repaired rule also records the exact Crowd Control fact once in `BonusSources` for combat-log attribution without applying a second bonus. | Final 0.0.87 player-attack-pipeline fixture proved exact +1 final AC and exact Crowd Control source attribution |
...
| Selection | Player-controlled | Three tier-specific parent selectors contain 6/10/15 exact children and owner reconciliation grants only the current tier. The rejected root `f431...` is hidden/inert; new roots are ordinary `1e55...`, Greater `92f8...`, and Mighty `f9a7...`. A primitive serialized `UnitPartControlledRageSelection` remains authoritative; stable selection facts remain synchronized carriers. | Final 0.0.87 actual live-grid fixture, persistence pair, CotW observers, and 18-state boundary PASS |
| Allocation icons | Immediately distinguish legal vectors | Pure choices reuse qualified Bull's Strength `4c3d...`, Cat's Grace `de7a...`, and Bear's Endurance `a900...` donors. Mixed icons render one miniature donor glyph for every +2 increment in deterministic two-, three-, or four-tile layouts. The live parent reflects the selected allocation; the selected child uses a 128x128 bright-green border/check overlay. | Final 0.0.87 fixture proved unique live icons and selected parent/child state; action-bar-size human judgment pending |
