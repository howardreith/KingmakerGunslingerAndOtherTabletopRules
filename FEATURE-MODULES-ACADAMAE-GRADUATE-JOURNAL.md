# Feature Modules, Acadamae Graduate, and Cord Journal

## 2026-08-09 — intake checkpoint

- Branch/HEAD: `codex/feature-modules-acadamae-graduate` / `7a99ce5ac6d6976212310f997bd39ddfe4a57935`.
- Active version: 0.0.74; informational version `0.0.74-paper-cartridges-auto-reload`.
- Starting state: canonical checkout clean; `origin/master` fetched and exactly matched HEAD; required branch did not previously exist and was created from exact origin/master.
- Baseline identities: blueprint manifest 249 stable identifiers, 248 active plus one reserved; bootstrap expected count 248.
- Baseline tests: repository validation PASS. First sandboxed domain run produced 953 PASS / 1 environmental UnauthorizedAccess failure in the SoundBank atomic replacement fixture. Exact rerun with the repository staging permissions PASS 954/954.
- Package/profile pins: active package `KingmakerGunslinger-0.0.74-local-runtime.zip`; exact local optional-mod hashes remain to be captured from `compatibility/references.json` and installed bytes.
- Work completed: read AGENTS.md and mandated intake documents/source; established durable mission, matrix, and initial boundary/prerequisite/Cord inventories.
- Rejected theory: ordinary `dotnet run` is not the repository test path; this classic net47 project must use `scripts/test-domain.ps1`.
- Current uncertainty: exact UMM 0.32.4 persistence API, installed wizard/action/condition/nonlethal contracts, and final capital owner graph.
- Next concrete action: inspect exact locally installed assemblies and complete the required contract inventories before production changes.

## 2026-08-09 — initial exact-contract checkpoint

- Branch/HEAD: `codex/feature-modules-acadamae-graduate` / `7a99ce5ac6d6976212310f997bd39ddfe4a57935`; version 0.0.74.
- UMM 0.32.4 exact signatures: `ModSettings.GetPath`, generic `Load<T>`, instance/static `Save`, `OnGUI(Action<ModEntry>)`, and `OnSaveGUI(Action<ModEntry>)`.
- Persistence decision: use project JSON plus atomic replacement because malformed-byte quarantine and precise recovery are mandatory; no dependency is added.
- Installed action surface: `AbilityData.ActionType`, `RuntimeActionType`, `RequireFullRoundAction`, real spellbook/slot/source/level members; `UnitUseAbility.Spell`, `m_CastTime`, and `OnAction`.
- Exact CotW: ID/version `CallOfTheWild` 1.14.4c-2.1; DLL SHA-256 `4EBF8E1ED3E66FFED72EA33EA325595629423DACD5BFFA23E3C9109144B26915`; no Acadamae/Cord name collision in its 2,987,468-byte blueprint ledger. It patches `AbilityData.ActionType`, `RuleCastSpell`, and `RuleApplyBuff` surfaces relevant to this mission.
- Native condition evidence: Fatigued and Exhausted exist as unit conditions and spell descriptors. `StatType.DamageNonLethal` exists in supporting source usage, so fallback selection remains premature pending installed event/serialization proof.
- Rejected theory: settings can safely be delegated blindly to UMM `ModSettings`; its XML helper does not itself establish the required malformed-byte retention contract.
- Current uncertainty: exact specialization/pending-level contracts, true multi-round cast representation, authoritative cancellation/completion correlation, and native nonlethal semantics.
- Next concrete action: finish exact IL/blueprint inventories for those four contracts, then implement the settings snapshot as the first narrow source checkpoint.

## 2026-08-09 — durable intake gate

- Branch/HEAD before commit: `codex/feature-modules-acadamae-graduate` / `7a99ce5ac6d6976212310f997bd39ddfe4a57935`; version 0.0.74.
- Required gates: `git diff --check` PASS; repository validation PASS; complete domain/reflection suite PASS 954/954; exact-reference Release build PASS; build-output, supply-icon, SoundBank, deterministic local package, and strict standalone UMM package validation PASS.
- Baseline package SHA-256: `67751efc238e8a55dfe6bedd375938f3e7df2fa3e3141c0e087502f9af0b1019`; DLL SHA-256: `9acfa985f9d27fa9c2e9c483a13a33ce184a235c48dd1c2cd9034c463beacd14`.
...
## 2026-08-09 — Acadamae action reduction and invocation-correlation checkpoint

- Branch/HEAD at implementation start: `codex/feature-modules-acadamae-graduate` / `c787705b2dc0da142a03ef4c472af98e69a9c97a`; active version 0.0.74.
- Exact installed IL proves full-round casting is an independent Boolean overlay on a Standard command: `AbilityData.RequireFullRoundAction` consults the final spell/action state, the public `UnitUseAbility(AbilityData, TargetWrapper)` obtains `RuntimeActionType`, and `UnitUseAbility.OnAction` synchronously creates/triggers `RuleCastSpell` only after availability/target/concentration cancellation gates. `RuleCastSpell.Success` is set only before real execution, and `UnitUseAbility.OnEnded(bool)` is the bounded cleanup surface.
- Exact CotW `1.14.4c-2.1` bytes (SHA-256 `4EBF8E1ED3E66FFED72EA33EA325595629423DACD5BFFA23E3C9109144B26915`) contain two `RequireFullRoundAction` postfixes and one `ActionType` postfix. The Acadamae postfix declares after-owner composition, consumes their final pre-Acadamae Boolean under a narrow inspection bypass, and changes only qualifying prepared arcane Conjuration/Summoning invocations whose exact spellbook can spend the slot.
- Added command-reference plus exact-`AbilityData` correlation: UI getter queries never arm state; only the authoritative constructor arms; `OnAction` creates a thread-scoped correlation; successful matching `RuleCastSpell` consumes exactly once; action exit/end/cancellation clears state. Unresolved spell levels and missing progression/slot availability fail closed.
- Tests/gates: deterministic suite PASS 966/966, including two commands, distinct same-looking prepared invocations, repeated arm/query behavior, exact single consumption, out-of-scope completion, and cancellation cleanup. Repository validation PASS; exact-reference clean Release, output/icon/SoundBank audits, deterministic package creation, and strict package validation PASS.
- Candidate package SHA-256 `cba51a5f2b6b2c1b7a8da13832b02ba4f2c625f07917b2aa64d21ce8ea4e8793`; DLL SHA-256 `4f875a3d5a1253b19699fa64db272c7f0ba252a69b20e311bf77a5644c3eb172`.
