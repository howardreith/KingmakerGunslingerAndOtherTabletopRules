...
$request = New-KmgRuntimeRequest -Scenario 'mod-load-smoke' -ExpectedVersion '0.0.57' `
...
Assert-Throws { New-KmgRuntimeRequest -Scenario 'unknown' -ExpectedVersion '0.0.57' `
...
Assert-Throws { New-KmgRuntimeRequest -Scenario 'mod-load-smoke' -ExpectedVersion '0.0.57' `
...
Assert-Throws { New-KmgRuntimeRequest -Scenario 'working-save-smoke' -ExpectedVersion '0.0.57' `
    -TimeoutSeconds 30 -ExitAfterCompletion $false -EvidenceDirectory $synthetic `
...
$featureRequest = New-KmgRuntimeRequest -Scenario 'generic-firearm-actions' `
    -ExpectedVersion '0.0.57' -TimeoutSeconds 30 -ExitAfterCompletion $true `
...
Assert-Throws { New-KmgRuntimeRequest -Scenario 'generic-firearm-actions' `
    -ExpectedVersion '0.0.57' -TimeoutSeconds 30 -ExitAfterCompletion $false `
...
Write-Host 'Runtime request source tests passed: 33'
