...
$package = if ($PackagePath) { $PackagePath } else { Get-KmgCompatibilityDefaultPackage $root }
$resolution = Resolve-KmgCompatibilityProfile -ProfileId $ProfileId -ReferenceRoot $ReferenceRoot -PackagePath $package -KingmakerInstallDir $KingmakerInstallDir -RepositoryRoot $root
