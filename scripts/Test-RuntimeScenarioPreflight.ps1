...
$expected = @(
    'mod-load-smoke',
    'observe-magic-circle-native-contracts', 'disposable-magic-circle-evil', 'disposable-magic-circle-ui',
...
Assert-Throws {
    New-KmgRuntimeRequest `
        -Scenario 'observe-kmg-compatibility-asset-attribution' `
        -ExpectedVersion '0.0.132' -TimeoutSeconds 120 `
...
foreach ($case in @('wrong-scenario','wrong-version','without-authority','mixed-authority','baseline','extra-parameter')) {
    ...
    switch ($case) {
        ...
        'wrong-version' { $bad117.ExpectedVersion='0.0.121' }
...
foreach ($race in @('Ifrit', 'Oread', 'Sylph', 'Undine')) {
    foreach ($allocation in @('point-buy', 'roll')) {
        [void](Assert-KmgRuntimeScenarioPreflight -Scenario 'disposable-elemental-character-creation-case' `
            -ExpectedVersion '0.0.132' -TimeoutSeconds 600 -StartupTimeoutSeconds 180 `
        ...
        foreach ($characterClass in @('Fighter', 'Gunslinger')) {
            $request = New-KmgRuntimeRequest -Scenario 'disposable-elemental-character-creation-case' `
                -ExpectedVersion '0.0.132' -TimeoutSeconds 600 -ExitAfterCompletion $true `
...
foreach ($invalid in @(
    ...
    Assert-Throws {
        Assert-KmgRuntimeScenarioPreflight -Scenario 'disposable-elemental-character-creation-case' `
            -ExpectedVersion '0.0.132' -TimeoutSeconds 600 -StartupTimeoutSeconds 180 -Parameters $invalid
...
foreach ($profileScenario in @('disposable-elemental-nereid-creation','disposable-elemental-nereid-respec')) {
    $profileArgs = @{ Scenario=$profileScenario; ExpectedVersion='0.0.132'; TimeoutSeconds=600; StartupTimeoutSeconds=300
    ...
    $outerOutput = & $orchestratorPath -Scenario $profileScenario -ExpectedVersion '0.0.132' -Parameters $profileArgs.Parameters `
...
foreach ($sex in @('Male','Female')) {
    ...
    $outerOutput = & $orchestratorPath -Scenario 'working-save-elemental-nereid-respec' -ExpectedVersion '0.0.132' `
...
foreach ($phase in @('elemental-race-persistence-prepare','elemental-race-module-disabled-persistence',
    ...
    $outerOutput = & $orchestratorPath -Scenario $phase -ExpectedVersion '0.0.132' -SaveName 'KMG_AUTOMATION_WORKING' `
...
foreach ($fixtureCase in @('public117','deferred117')) {
    ...
    $outerOutput = & $orchestratorPath -Scenario $creatorArgs.Scenario -ExpectedVersion '0.0.132' -SaveName 'KMG_AUTOMATION_WORKING' `
...
$offCreatorArgs=@{Scenario='disposable-elemental-character-creation-baseline';ExpectedVersion='0.0.132';TimeoutSeconds=600;Parameters=@{creatorCase='module-off'}}
...
$outerOutput = & $orchestratorPath -Scenario 'disposable-elemental-character-creation-baseline' -ExpectedVersion '0.0.132' `
