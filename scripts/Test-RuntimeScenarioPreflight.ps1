...
$assetRequest = New-KmgRuntimeRequest `
    ...
    -ExpectedVersion '0.0.126' -TimeoutSeconds 120 -ExitAfterCompletion $true `
...
Assert-Throws {
    New-KmgRuntimeRequest `
        -Scenario 'observe-kmg-compatibility-asset-attribution' `
        -ExpectedVersion '0.0.126' -TimeoutSeconds 120 `
...
try {
    Assert-Throws {
        & $orchestratorPath -Scenario 'unsupported-regression-fixture' `
            -ExpectedVersion '0.0.126' -WhatIf -Confirm:$false
...
foreach ($race in @('Ifrit', 'Oread', 'Sylph', 'Undine')) {
    foreach ($allocation in @('point-buy', 'roll')) {
        [void](Assert-KmgRuntimeScenarioPreflight -Scenario 'disposable-elemental-character-creation-case' `
            -ExpectedVersion '0.0.126' -TimeoutSeconds 600 -StartupTimeoutSeconds 180 `
        ...
        foreach ($characterClass in @('Fighter', 'Gunslinger')) {
            $request = New-KmgRuntimeRequest -Scenario 'disposable-elemental-character-creation-case' `
                -ExpectedVersion '0.0.126' -TimeoutSeconds 600 -ExitAfterCompletion $true `
...
foreach ($invalid in @(
    ...
    Assert-Throws {
        Assert-KmgRuntimeScenarioPreflight -Scenario 'disposable-elemental-character-creation-case' `
            -ExpectedVersion '0.0.126' -TimeoutSeconds 600 -StartupTimeoutSeconds 180 -Parameters $invalid
...
foreach ($profileScenario in @('disposable-elemental-nereid-creation','disposable-elemental-nereid-respec')) {
    $profileArgs = @{ Scenario=$profileScenario; ExpectedVersion='0.0.126'; TimeoutSeconds=600; StartupTimeoutSeconds=300
    ...
    $outerOutput = & $orchestratorPath -Scenario $profileScenario -ExpectedVersion '0.0.126' -Parameters $profileArgs.Parameters `
...
foreach ($sex in @('Male','Female')) {
    ...
    $outerOutput = & $orchestratorPath -Scenario 'working-save-elemental-nereid-respec' -ExpectedVersion '0.0.126' `
...
foreach ($phase in @('elemental-race-persistence-prepare','elemental-race-module-disabled-persistence',
    ...
    $outerOutput = & $orchestratorPath -Scenario $phase -ExpectedVersion '0.0.126' -SaveName 'KMG_AUTOMATION_WORKING' `
...
foreach ($fixtureCase in @('public117','deferred117')) {
    ...
    $outerOutput = & $orchestratorPath -Scenario $creatorArgs.Scenario -ExpectedVersion '0.0.126' -SaveName 'KMG_AUTOMATION_WORKING' `
...
$offCreatorArgs=@{Scenario='disposable-elemental-character-creation-baseline';ExpectedVersion='0.0.126';TimeoutSeconds=600;Parameters=@{creatorCase='module-off'}}
...
$outerOutput = & $orchestratorPath -Scenario 'disposable-elemental-character-creation-baseline' -ExpectedVersion '0.0.126' `
