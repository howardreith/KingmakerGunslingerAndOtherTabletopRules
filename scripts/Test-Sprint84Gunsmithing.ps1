...
Require ($bootstrap.Contains('ExpectedRegisteredBlueprintCount = 128') -and $bootstrap.IndexOf('AttachReload') -lt $bootstrap.IndexOf('GunsmithingBlueprints.Register')) 'bootstrap-count-and-order'
...
Require (@($manifest.entries).Count -eq 128) 'ledger-count'
Require (@($manifest.entries | Where-Object status -eq 'active').Count -eq 128) 'active-count'
