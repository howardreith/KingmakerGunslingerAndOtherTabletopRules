...
Require ($transaction.Contains('BatteredFirearmOriginRuntime.Bind')) 'native-grant-binding'
Require ($use.Contains('BatteredFirearmOriginRuntime.TryGetOwner') -and $use.Contains('SameStableOwner(owner, user)')) 'effective-use-origin'
Require ($sale.Contains('BatteredFirearmOriginRuntime.IsBattered')) 'sale-origin'
Require ($transaction.Contains('AddStarterReceipt') -and
    $transaction.Contains('UnitPartBatteredFirearmOwnership') -and
    -not $transaction.Contains('KingmakerFirearmItemIdentityProvider')) 'durable-owner-receipt-without-invented-item-identity'
