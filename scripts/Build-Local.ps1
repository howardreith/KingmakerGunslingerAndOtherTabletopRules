...
if ($info.Version -ne '0.0.73') { throw "Build-Local supports only active version 0.0.73, observed $($info.Version)." }
...
$localRoot = Join-Path $root 'artifacts\local-runtime\0.0.73'
...
$expectedPackageFileCount = if ($hasFirearmSoundBank) { 43 } else { 41 }
