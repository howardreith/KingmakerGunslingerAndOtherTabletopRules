...
if ($info.Version -ne '0.0.138') { throw "Build-Local supports only active version 0.0.138, observed $($info.Version)." }
...
$localRoot = Join-Path $root 'artifacts\local-runtime\0.0.138'
...
$expectedPackageFileCount = if ($hasFirearmSoundBank) { 244 } else { 242 }
